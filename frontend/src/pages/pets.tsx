...
const Pets: NextPage = () => {
  ...
  const validationSchema: yup.AnyObjectSchema = yup.object().shape({
    ...
    birthday: yup
      ...
      .nullable()
      .min(
        moment().subtract(153, "years"),
        "Aniversário não pode ser pré-histórico."
      )
    ...
    >
      <Formik
        ...
        onSubmit={async (values: FormValues, helpers) => {
          ...
          const submitted: boolean = item?.id
            ? await PetService.update(item.id, data)
                  ...
                  <DatePicker
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        helperText={
                          Boolean(errors.birthday) ? errors.birthday : ""
                        }
                      />
                    )}
                  />
